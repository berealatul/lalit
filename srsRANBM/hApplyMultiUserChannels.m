    ...
    for ch = scheduledUEs
        ...
        if (nD~=0 && ismember(ch,schedule.PDSCH))
            ...
            if (channel.ChannelFiltering)
                ...
                s = applyFrequencyDomainChannel(carrier,channels(ch).channel,dlGrid,ofdmInfo);
            end
        ...
        if (nU~=0 && ismember(ch,schedule.SRS))
            ...
            if (channel.ChannelFiltering)
                ...
                channel.NumTimeSamples = nD + nG + nU;
                s = applyFrequencyDomainChannel(carrier,channels(ch).channel,ulGrid,ofdmInfo);
            end
...
function [channels,maxDelay] = setupChannels(carrier,channels,scheduledUEs,ofdmInfo)
    ...
    for ch = scheduledUEs
        ...
        delay = chInfo.MaximumChannelDelay;
...
function s = newOutputStruct()
    ...
    s = struct('rxWaveform',[],'rxGrid',[],'ofdmInfo',[],'pathGains',[],'sampleTimes',[],'noisePower',[],'ChannelFilterDelay',[],'pathFilters',[]);
...
function s = applyFrequencyDomainChannel(carrier,channel,txGrid,ofdmInfo)
    ...
    rxGrid = hApplyFrequencyDomainChannel(carrier,pathGains,pathFilters,sampleTimes,txGrid,offset);
