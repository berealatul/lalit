...
%% SLOT TYPE DETERMINATION
    ...
    slotType = tddPattern(mod(carrier.NSlot,numel(tddPattern))+1); %numel--return no. of element 
    % (10) in array tddpattern   %carrier.Nslot=39 %mod(39,10)=9
    % %slotType=tddpattern(10)
    if (slotType=="D")
        nSymbolsDL = carrier.SymbolsPerSlot; %symbolperslot=14
    elseif (slotType=="S")
        nSymbolsDL = specialSlot(1);  %specialslot=[6 4 4] [Dl GB Ul]
    else % slotType=="U"
        nSymbolsDL = 0;
    end
%% CHECK FOR NO SCHEDULULING DONE IF NO SYMBOL FOR DL or NO CSI  

    % If there are no downlink symbols or no CSI has been recorded yet, no
    % PDSCH can be scheduled in this slot
    noCSI = all(isnan(csi(1).H(:,:,1,1))); %all-- give true if all non-zero element in col
    if (nSymbolsDL==0 || noCSI)
        schedule.PDSCH = zeros(1,0);
        B = [];
        return;
    end
%% REPLACE Not_a_Number(NaN) 0/0 to nearest non_NaN value
    ...
    numUEs = numel(PDSCHs);
    for ue = 1:numUEs
        nanidx = find(isnan(csi(ue).H(:,:,1,1)));      %csi(ue).H(:,:,1,1) -- csi for each UE
        %isnan-- determine which array elements are NaN
        %nanidx -- indices where csi is invalid number
        nonnanidx = find(~isnan(csi(ue).H(:,:,1,1)));
        %nonnanidx -- indices where csi is valid number
...
%% Layer and Group Initialization

    % Extract number of layers from the PDSCH configurations and compute
    % total number of layers
    numLayers = [cat(1,PDSCHs.Config).NumLayers]; % Concatanate all UE layer in col
    ...
    if ~isfield(alg,'groups')
        groups = ones(1,numUEs);
          ...
          % group=alg.groups;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 groups = alg.groups;
    end
    G = max(groups);

    isJSDMActive = any(strcmp(alg.PrecodingMethod,{'JSDM-JGP','JSDM-PGP'}));
    %tf = strcmp(s1,s2) compares s1 and s2 and 
    % returns 1 (true) if the two are identical and 0 (false) otherwise.
    %any-- logical OR if any one element is non-zero return true
    if isJSDMActive
...
%% PRG-Based UE Selection
    ...
    allW = zeros(prgInfo.NPRG,alg.ScheduledLayers,numRF);
...
%% Channel & Noise Estimation for PRG
...
%% Precoding and Capacity Calculation
...
%% UE Sorting and DM-RS Configuration
...
%% Final Scheduling and PDSCH Configuration
    ...
    for ue = schedule.PDSCH
        ...
        PDSCHs(ue).Extension.W = zeros(nu,numRF,prgInfo.NPRG);
